...
void Matrix::decomposeLU(vector<unsigned int>& outIndexes, int& outD)
{
	...
	for(unsigned int j = 0; j < mCols; ++j) {
		...
		if(j != (mCols-1)) {
			...
			{
			for(unsigned int i = j+1; i < mRows; ++i){
				(*this)(i,j) *= lDummy;
			}
			}
		}
...
Matrix& Matrix::invert(Matrix& outMatrix) const
{
	...
	for(unsigned int j = 0; j < mCols; ++j) {
		{
		for(unsigned int i = 0; i < mRows; ++i){
			lB(i,0) = outMatrix(i,j);
		}
		}
		lTmp.computeBackSubLU(lIndexes, lB);
		{
		for(unsigned int i = 0; i < mRows; ++i){
			outMatrix(i,j) = lB(i,0);
		}
		}
	}
	}
...
ostream& PACC::operator<<(ostream &outStream, const Matrix& inMatrix)
{
	...
	{
	for(unsigned int i = 0; i< inMatrix.getRows(); ++i) 
	{
		{
		for(unsigned int j = 0; j < inMatrix.getCols(); ++j) 
		{
			if(inMatrix(i,j) > 0.0)
			{
				outStream<<" "<<std::scientific<< inMatrix(i,j)<<" ";
			}
			else if(inMatrix(i,j) < 0.0)
			{
				outStream<<std::scientific<< inMatrix(i,j)<<" ";
			}
			else
